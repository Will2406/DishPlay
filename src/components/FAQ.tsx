...
export default function FAQ() {
  ...
  return (
    <section ref={ref} id="contacto" className="bg-cream dark:bg-dark-deep py-20 md:py-32">
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        <motion.div {...anim(0)} className="text-center mb-14">
          <h2 className="heading text-[clamp(2.5rem,6vw,4rem)] text-text-primary dark:text-text-on-dark mb-3">
            Se de los <span className="accent-serif text-brand-red">primeros.</span>
          </h2>
          <p className="font-body text-lg text-text-secondary dark:text-text-on-dark-secondary max-w-xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          <div>
            <h3 className="font-body font-bold text-text-primary dark:text-text-on-dark mb-4">Preguntas frecuentes</h3>
              ...
              {faqs.map((faq, i) => (
                <motion.div key={i} {...anim(0.15 + i * 0.05)} className="card-soft overflow-hidden">
                  <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left" aria-expanded={open === i}>
                    <span className="font-body text-sm font-semibold text-text-primary dark:text-text-on-dark pr-3">{faq.q}</span>
                  ...
                  <AnimatePresence>
                    {open === i && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                        <p className="px-4 pb-4 font-body text-sm text-text-secondary dark:text-text-on-dark-secondary">{faq.a}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </motion.div>
          ...
          >
            <h3 className="heading text-xl text-text-primary dark:text-text-on-dark mb-1">Unete a la lista de espera</h3>
            <p className="font-body text-sm text-text-secondary dark:text-text-on-dark-secondary mb-5">Te avisamos cuando estemos listos para visitarte.</p>
            ...
            <div className="flex flex-col gap-3.5">
              {[
                ...
                <div key={f.id}>
                  <label htmlFor={f.id} className="block font-body text-xs font-medium text-text-primary dark:text-text-on-dark mb-1 uppercase tracking-wider">{f.label}</label>
                  <input id={f.id} name={f.id} type="text" autoComplete={f.auto} placeholder={f.ph}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-border-light dark:border-border-dark bg-white dark:bg-white/5 font-body text-sm text-text-primary dark:text-text-on-dark placeholder:text-text-muted dark:placeholder:text-text-on-dark-secondary/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-red/20 focus-visible:border-brand-red transition-[border,box-shadow]"
                    required
                  />
                </div>
              ))}
              ...
              <div>
                <label htmlFor="contacto" className="block font-body text-xs font-medium text-text-primary dark:text-text-on-dark mb-1 uppercase tracking-wider">WhatsApp</label>
                <div className="flex rounded-xl border border-border-light dark:border-border-dark focus-within:ring-2 focus-within:ring-brand-red/20 focus-within:border-brand-red transition-[border,box-shadow] overflow-hidden">
                  <span className="flex items-center px-3 bg-cream dark:bg-white/5 font-body text-sm text-text-secondary dark:text-text-on-dark-secondary border-r border-border-light dark:border-border-dark select-none">+51</span>
                  <input id="contacto" name="contacto" type="tel" autoComplete="tel" placeholder="999 999 999"
                    value={form.contacto}
                    onChange={(e) => { const digits = e.target.value.replace(/\D/g, "").slice(0, 9); setForm({ ...form, contacto: digits }); }}
                    maxLength={9} inputMode="numeric" pattern="[0-9]{9}"
                    className="flex-1 px-3 py-3 font-body text-sm text-text-primary dark:text-text-on-dark placeholder:text-text-muted dark:placeholder:text-text-on-dark-secondary/50 focus-visible:outline-none bg-transparent"
                    required
                  />
                </div>
              </div>
            ...
            <p className="font-body text-[11px] text-text-muted dark:text-text-on-dark-secondary/50 text-center mt-3">Sin spam. Sin compromiso. Solo te escribimos cuando estemos listos.</p>
          </motion.form>
        </div>

        <motion.div {...anim(0.5)} className="mt-10 text-center">
          <a href="https://wa.me/51998027578?text=Hola%20quiero%20saber%20mas%20sobre%20DISHPLAY" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 font-body font-semibold text-sm text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 transition-colors"
          >
