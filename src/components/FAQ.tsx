...
const faqs = [
  { q: "¿Mis clientes necesitan descargar una app?", a: "No. La carta abre directo desde la camara al escanear el QR. Funciona en iOS y Android." },
  { q: "¿Tengo que llevar mis platos a algun lado?", a: "Para nada. Vamos a tu local con todo lo necesario. Tu restaurante sigue operando." },
  { q: "¿En cuanto tiempo tengo mi carta lista?", a: "Entre 5 y 7 dias habiles desde la sesion." },
  { q: "¿Puedo actualizar mi carta yo mismo?", a: "Si. Panel admin para activar/desactivar platos, cambiar precios y descripciones." },
  { q: "¿Atienden fuera de Lima?", a: "Por ahora Lima Metropolitana. Escribenos si estas en otra ciudad." },
];
...
export default function FAQ() {
  ...
  const [form, setForm] = useState({ nombre: "", restaurante: "", contacto: "", plan: "" });
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        ...
        <motion.div {...anim(0)} className="text-center mb-14">
          <h2 className="heading text-[clamp(2.5rem,6vw,4rem)] text-text-primary mb-3">
            Empieza <span className="accent-serif text-brand-red">hoy.</span>
          </h2>
          <p className="font-body text-lg text-text-secondary max-w-md mx-auto">
            En 5 dias, tus clientes ya viven la experiencia.
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          {/* FAQ + image */}
          <div>
            <motion.div {...anim(0.1)} className="rounded-2xl overflow-hidden shadow-lg mb-8">
              <Image
                src="https://images.unsplash.com/photo-1551888797-ec22463d8461?w=700&h=350&fit=crop&q=80"
                alt="Dueno de restaurante sonriendo mientras prepara masa"
                width={700} height={350} className="w-full h-auto object-cover"
              />
            </motion.div>
          ...
          >
            <h3 className="heading text-xl text-text-primary mb-5">Contactanos</h3>
            ...
            <div className="flex flex-col gap-3.5">
              ...
              <div>
                <label htmlFor="plan" className="block font-body text-xs font-medium text-text-primary mb-1 uppercase tracking-wider">Plan de interes</label>
                <select id="plan" name="plan" autoComplete="off" value={form.plan} onChange={(e) => setForm({ ...form, plan: e.target.value })}
                  className="w-full px-4 py-3 rounded-xl border border-border-light font-body text-sm text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-red/20 focus-visible:border-brand-red appearance-none bg-white"
                >
                  <option value="">Selecciona...</option>
                  <option value="esencial">Esencial</option>
                  <option value="inmersivo">Inmersivo</option>
                  <option value="no-se">No se aun</option>
                </select>
              </div>
              ...
              <button type="submit" className="w-full bg-brand-red text-white font-body font-bold text-sm py-3.5 rounded-full hover:bg-brand-red-dark transition-[background,box-shadow] hover:shadow-lg hover:shadow-brand-red/20 active:scale-[0.98] mt-1">
                {sent ? "¡Enviado! ✓" : "Quiero que me contacten →"}
              </button>
            </div>

            <p className="font-body text-[11px] text-text-muted text-center mt-3">Sin spam. Sin presion.</p>
          </motion.form>
        </div>
