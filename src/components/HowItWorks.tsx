...
export default function HowItWorks() {
  ...
  return (
    <section ref={ref} id="como-funciona" className="bg-dark-surface py-20 md:py-32">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <motion.div {...anim(0.1)} className="relative order-2 lg:order-1">
            ...
            <motion.div
              ...
              className="absolute -bottom-4 -right-3 md:-right-6 glass-dark rounded-[1.25rem] px-5 py-3 flex items-center gap-3"
            >
              <span className="heading text-2xl text-brand-red" style={{ fontVariationSettings: '"SOFT" 100' }}>5</span>
              <div>
                <p className="text-xs font-bold text-text-on-dark">dias habiles</p>
                <p className="text-[10px] text-text-on-dark-secondary">y tu carta esta lista</p>
              </div>
            </motion.div>
          </motion.div>
          ...
          <div className="order-1 lg:order-2">
            <motion.div {...anim(0)}>
              <span className="inline-block bg-accent-orange/15 text-accent-orange font-body font-semibold text-xs tracking-wide uppercase px-3.5 py-1.5 rounded-full mb-5">
                Proceso simple
              </span>
              <h2 className="heading text-[clamp(2rem,5vw,3rem)] text-text-on-dark mb-8">
            ...
            <div className="flex flex-col gap-3">
              {steps.map((s, i) => (
                <motion.div key={s.n} {...anim(0.15 + i * 0.1)}
                  className={`flex gap-4 p-4 rounded-xl ${s.highlight ? "bg-brand-red/10" : "bg-white/5"}`}
                >
                  <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-sm font-body font-bold ${
                    s.highlight ? "bg-brand-red text-white" : "bg-white/10 text-text-on-dark-secondary"
                  }`}>
                    {s.n}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="font-body font-bold text-text-on-dark text-sm">{s.title}</h3>
                      <span className="text-[10px] font-body text-text-on-dark-secondary/60">· {s.time}</span>
                    </div>
                    <p className="font-body text-sm text-text-on-dark-secondary">{s.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>

            <motion.div {...anim(0.6)} className="flex flex-wrap gap-x-5 gap-y-1 mt-6 text-xs font-body text-text-on-dark-secondary">
            ...
            <motion.div {...anim(0.65)} className="mt-5">
              <a href="#contacto" className="inline-block bg-brand-red text-white font-body font-bold text-sm px-7 py-3.5 rounded-full hover:bg-brand-red-dark transition-[background] active:scale-[0.98]">
